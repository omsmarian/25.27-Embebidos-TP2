...
class GUI(tk.Tk):
	...
	def __create_rotation_value_boxes(self):
		...
		self.x_rot_value = ttk.Label(self, text=str(self.x_rotation_slider.get()))
		self.x_rot_value.place(relx=self.COMMON_X, rely=0.525, relheight=0.04, relwidth=0.1, anchor="ne")

	def update_rotation_values(self, x, y, z):
		self.x_rot_value.delete(0, tk.END)
		self.x_rot_value.insert(0, str(x))
		self.y_rot_value.delete(0, tk.END)
		self.y_rot_value.insert(0, str(y))
		self.z_rot_value.delete(0, tk.END)
		self.z_rot_value.insert(0, str(z))
