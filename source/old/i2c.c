/***************************************************************************//**
  @file     i2c.c
  @brief    I2C driver for K64F. Non-blocking, multiple I2C modules supported
...
// Find closest baud rate to desiredBaudeRate
BaudRate_t SetBaudRate(uint32_t desiredBaudRate)
{
	BaudRate_t settings = {.mul = 0, .icr = 0};
	uint32_t bestBaudRate = 0, error = 0, currentBaudRate = 0;
	uint8_t mul, icr;
...
void I2C_IRQHandler(I2C_Module_t module)
{
	...
	if(I2C_GET_TX_MODE)
	{
		...
		if(I2C_MODE == I2C_Write)
		{
			...
			{
				// RXAK = 0 OK, RXAK = 1 ERROR
				if(!I2C_GET_RX_ACK)
				{
					// If reg_address was not sent
					if(!I2C_REG_ADDRESS_SENT)
					{
						I2C_WRITE_DATA(I2C_REG_ADDRESS);
						I2C_REG_ADDRESS_SENT = true;
					}
					// Send next byte if addr was already sent
					else
					{
		...
		else if(I2C_MODE == I2C_Read)
		{
			...
			if(!I2C_GET_RX_ACK)
			{
				...
				// If register address was already sent, send repeated start
				else
				{
					// If repeated start was not sent
					if(!I2C_REPEATED_START_SENT)
					{
						I2C_REPEAT_START_SIGNAL;
						I2C_WRITE_DATA(I2C_ADDRESS_MASK | 0x00000001); // Could be a macro
	...
	{
		// Size - 1 = last byte to read
		// If byte to read is the last
